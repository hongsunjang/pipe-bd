...
class EfficientNet(nn.Module):
    ...
    def forward_features(self, x, block_idx = None):
        if block_idx is None:
            ...
            if (self.conv_stem is not None) and (block_idx == 0):
                x = self.conv_stem(x)
                x = self.bn1(x)
                x = self.first_block(x)
            ...
            if (self.conv_head is not None) and (block_idx + 1 == len(self.blocks)):
                x = self.conv_head(x)
                x = self.bn2(x)
    ...
    def forward(self, x, block_idx = None):
        if block_idx is None:
            x = self.forward_features(x, block_idx)
            if (self.conv_head is not None):
