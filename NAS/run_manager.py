...
class RunConfig:
    ...
    def __init__(self, n_epochs, init_lr, lr_schedule_type, lr_schedule_param,
                 ...
                 model_init, init_div_groups, validation_frequency, print_frequency, train_method='dp'):
...
class RunManager:
    ...
    def __init__(self, path, net, run_config: RunConfig, out_log = True, stage = 0, rank_to_feature_shape = None, rank_to_pgroup = None, skip_init = False, measure_latency=None):
        self.path = path
        self.net = net
        self.run_config = run_config
        self.rank_to_feature_shape = rank_to_feature_shape
        ...
        if skip_init == False and hasattr(self.net, 'init_model'):
        ...
        if torch.cuda.is_available():
            self.device = int(torch.distributed.get_rank())
            self.net = self.net.to(self.device)
            if self.run_config.train_method is None or self.run_config.train_method == 'ts':
                pass
            elif  self.run_config.train_method == 'dp':
                self.net = DDP(self.net, device_ids=[self.device], find_unused_parameters=True)
            elif self.run_config.train_method == 'dna' and rank_to_feature_shape is not None:
                if self.net.first_conv is not None:
                    self.net.first_conv = DDP(self.net.first_conv, device_ids=[self.device], find_unused_parameters=True,)

                self.net.blocks = torch.nn.ModuleList([ DDP(block, device_ids=[self.device], find_unused_parameters=True)  for block in self.net.blocks ])
                 
                if self.net.feature_mix_layer is not None:
                    self.net.feature_mix_layer  = DDP(self.net.feature_mix_layer, device_ids=[self.device], find_unused_parameters=True,)
                    self.net.classifier = DDP(self.net.classifier, device_ids=[self.device], find_unused_parameters=True,)
            elif self.run_config.train_method == 'mp' and rank_to_pgroup is not None:
                # Using torch.nn.parallel is mandatory
                if rank_to_pgroup[int(self.device)] is not None:
                    if self.net.first_conv is not None:
                        self.net.first_conv = DDP(self.net.first_conv, device_ids=[self.device], find_unused_parameters=True, process_group= rank_to_pgroup[int(self.device)])
                    self.net.blocks = torch.nn.ModuleList([ DDP(block, device_ids=[self.device], find_unused_parameters=True, process_group = rank_to_pgroup[int(self.device)])  for block in self.net.blocks ])
                
                    if self.net.feature_mix_layer is not None:
                        self.net.feature_mix_layer = DDP(self.net.feature_mix_layer, device_ids=[self.device], find_unused_parameters=True, process_group= rank_to_pgroup[int(self.device)])
                        self.net.classifier = DDP(self.net.classifier, device_ids=[self.device], find_unused_parameters=True, process_group= rank_to_pgroup[int(self.device)])
        ...
        # net info
        if hasattr(self.net, 'get_flops') and self.run_config.train_method != 'ts':
            self.print_net_info(rank_to_feature_shape, measure_latency)
    ...
    def net_flops(self):
        if self.rank_to_feature_shape is None:
            data_shape = [1] + list(self.run_config.data_provider.data_shape)
        else:
            data_shape = list(self.rank_to_feature_shape[self.stage])
    ...
    def print_net_info(self, rank_to_feature_shape= None, measure_latency=None):
    ...
    def write_log(self, log_str, prefix, should_print=True):
        ...
        if prefix in ['valid', 'test', 'train']:
            with open(os.path.join(self.logs_path, 'train_console.txt'), 'a') as fout:
                ...
                fout.flush()
        if prefix in ['time']:
            with open(os.path.join(self.logs_path, 'time_console.txt'), 'a') as fout:
                fout.write(log_str + '\n')
                fout.flush()
        if should_print:
            print(log_str)
    ...
    def validate(self, is_test=True, net=None, use_train_mode=False, return_top5=False):
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(data_loader):
                ...
                if self.out_log and (i % self.run_config.print_frequency == 0 or i + 1 == len(data_loader)):
                    if is_test:
                        prefix = 'Test'
                    else:
                        prefix = 'Valid'
    ...
    def train_one_epoch(self, adjust_lr_func, train_log_func):
        ...
        for i, (images, labels) in enumerate(self.run_config.train_loader):
            ...
            if self.out_log and (i % self.run_config.print_frequency == 0 or i + 1 == len(self.run_config.train_loader)):
