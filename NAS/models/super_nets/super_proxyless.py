...
class SuperProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, width_stages, n_cell_stages, conv_candidates, stride_stages,
        ...
        for width, n_cell, s in zip(width_stages, n_cell_stages, stride_stages):
            ...
            for i in range(n_cell):
                ...
                stage.append(inverted_residual_block)
                input_channel = width
        ...
        super(SuperProxylessNASNets, self).__init__(stem, blocks, feature_mix_layer, classifier)
    ...
    def expected_flops(self, x, block_idx = None):
        if block_idx is None:
            expected_flops = 0
            # first conv
            if self.first_conv is not None:
                for conv in self.first_conv:
                    flop, x = conv.get_flops(x)
                    expected_flops += flop
