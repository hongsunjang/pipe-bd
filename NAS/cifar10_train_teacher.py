...
if __name__ == '__main__':
    ...
    # prepare run config
    """
    run_config_path = os.path.join(args.path,'run.config')
    if os.path.isfile(run_config_path):
        raise NotImplementedError
        # load run config from file
        run_config = json.load(open(run_config_path, 'r'))
        run_config = Cifar10RunConfig(**run_config)
        if args.valid_size:
    ...
    #TODO: do not use last block of teacher blocks
    net = MobileProxylessNASNets(
    ...
    """
    init_path = os.path.join(args.path,'init')
        ...
        if 'proxyless' in args.net and not args.train:
            raise NotImplementedError
